...
def setup(app):
    app.add_directive('petljaactivecode', PetljaActiveCode)
    app.add_directive('actex', PetljaActiveExercise)
    app.add_role('textfield',textfield_role)
    app.add_config_value('petljaactivecode_div_class', "runestone explainer ac_section alert alert-warning", 'html')
    app.add_config_value('petljaactivecode_hide_load_history', False, 'html')

    app.add_stylesheet('petljaactivecode.css')
    ...
    add_i18n_js(app, {"en","sr-Cyrl"},"petljaactivecode-i18n")
    add_skulpt_js(app)
    app.add_javascript('petljaactivecode.js')
    app.add_javascript('clike.js')
    app.add_javascript('timed_petljaactivecode.js')
    ...
    app.connect('env-purge-doc', purge_petljaactivecodes)
...
TEMPLATE_END = """
<textarea data-component="petljaactivecode" id=%(divid)s data-lang="%(language)s" %(autorun)s
...
def purge_petljaactivecodes(app, env, docname):
    pass

class PetljaActiveCode(RunestoneIdDirective):
    """
.. petljaactivecode:: uniqueid
...
- petljaactivecode_div_class - custom CSS class of the component's outermost div
- petljaactivecode_hide_load_history - if True, hide the load history button
    """
    ...
    def run(self):
        super(PetljaActiveCode, self).run()
        ...
        if not hasattr(env, 'petljaactivecodecounter'):
            env.petljaactivecodecounter = 0
        env.petljaactivecodecounter += 1
        ...
        self.options['divclass'] = env.config.petljaactivecode_div_class
        if env.config.petljaactivecode_hide_load_history:
        ...
        if engine:
            ...
            if not hasattr(env, 'dberr_petljaactivecode_reported') or not env.dberr_petljaactivecode_reported:
                env.dberr_petljaactivecode_reported = True
                print("Unable to save to source_code table in petljaactivecode.py. Possible problems:")
...
class PetljaActiveExercise(PetljaActiveCode):
    required_arguments = 1
    ...
    def run(self):
        ...
        self.options['gradebutton'] = "data-gradebutton=true"
        self.options['coach'] = "data-coach=true"
        return super(PetljaActiveExercise, self).run()
